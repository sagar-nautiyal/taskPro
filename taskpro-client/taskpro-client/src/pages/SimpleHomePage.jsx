...
export const SimpleHomePage = () => {
  ...
  const { boards, loading: boardsLoading } = useSelector(state => state.board);
    ...
    <div style={{ minHeight: '100vh' }}>
      ...
      <header style={{ 
        ...
        <div className="container" style={{ 
          ...
          <h1 style={{ fontSize: '1.5rem', color: 'var(--primary)' }}>TaskPro</h1>
      ...
      <div style={{ background: 'var(--bg-secondary)', padding: '1rem' }}>
        <div className="container">
          <div style={{ display: 'flex', gap: '1rem' }}>
            <button 
              className={`btn ${activeView === 'tasks' ? 'btn-primary' : 'btn-secondary'}`}
              onClick={() => setActiveView('tasks')}
            >
              Tasks
            </button>
            <button 
              className={`btn ${activeView === 'boards' ? 'btn-primary' : 'btn-secondary'}`}
                      ...
                      {tasksByStatus.completed.length === 0 ? (
                        ...
                        tasksByStatus.completed.map(task => (
                          <div key={task._id} style={{ 
                            ...
                            <h4 style={{ marginBottom: '0.25rem', textDecoration: 'line-through', opacity: 0.7 }}>
                              {task.title}
                            </h4>
                            <p className="text-muted" style={{ fontSize: '0.875rem' }}>
                              {task.description}
                      ...
                      <div key={board._id} className="card">
                        ...
                        <div style={{ marginTop: '1rem' }}>
                          <small className="text-muted">
                            {board.lists?.reduce((total, list) => total + (list.tasks?.length || 0), 0)} tasks
                          </small>
                        </div>
                      </div>
                    ))
